...
// async function addMap() {
    ...
    hoveredId = null;
    dir = 'http://glo-repetitiveloss.s3-website.us-east-2.amazonaws.com/'
    lyrdir = dir +'bringtheheat/'
    huclyrdir = dir+'HUC/'
    ...
    fly = async (map,x,y,z,sleep=2000)=> {
    ...
    loadErUp = async () => { //had to strip out to separate func to reload after style/basemap change 
        ...
        zip = (a, b) => Array.from(a).map((k, i) => [k, Array.from(b)[i]])
        fldzonelyrs = ['500YR Preliminary',
                    '500YR Effective',
                    '100YR Preliminary',
                    '100YR Effective']
        colrs = zip(fldzonelyrs,['#c36c08','#c7af0e','#0000b0','#5544fd'])
        ...
        replossId = 'FEMA Severe Repetitive Loss Properties'
        let heatmaplyrs = [
            {id:replossId,
            data:reptLossData,
        ...
        map.on('mousemove', 'FEMA Severe Repetitive Loss Properties-point', (event) => {
            ...
            .setHTML(`<strong>Repetitive Loss Properties:</strong> ${event.features[0].properties.reptloss}<br>
                    in zip code ${event.features[0].properties.id}`
                    )
            .addTo(map);
        ...
        map.on('mousemove', 'zipcodes', (e) => {
            if (e.features.length > 0 & playing==false) {
